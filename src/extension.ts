...
function buildNitpickFixerAgent(
  ...
  // LinterAdapter — bypasses LinterIntegrationService detection (npx --version fails
  // in the VS Code extension host) and calls linters directly via TerminalMCPClient.
  // ESLint and Prettier run directly via child_process — bypasses TerminalMCPClient
  // allowlist/spawn restrictions that cause failures in the extension host.
  const linterAdapter = {
    ...
    try {
      ...
      switch (result.status) {
        ...
        case 'rejected':
          nitpickPanel.showSuccess(result);
          vscode.window.showInformationMessage(
            'DevMind: Nitpick fixes rejected — no changes applied.'
          );
          break;
        case 'no_linters':
          nitpickPanel.showError(result.errorMessage ?? 'No linters detected in this project.');
          vscode.window.showWarningMessage(
            'DevMind: No linters detected. Add .eslintrc or .prettierrc.'
          );
          break;
        case 'failed':
